...
aux1<-cs12p_8 %>% filter(p23 == 1 & p01%in% 1:5) %>% select(llave, p23, p24)
length(unique(aux1$llave))

if(3==2){
# explorar las llaves repetidas
rep(c(1:3),c(2,5,1))
repetidos<-aux1 %>% group_by(llave) %>% count() %>% filter(n>1)
cs12p_8 %>% filter(llave=="080202002608") %>% View()
cs12p_8 %>% filter(llave %in% repetidos$llave) %>% group_by(p01) %>% count()
}
...
v8<-v8 %>% filter(p02 %in% 0:1)
table(is.na(v8$p04))

length(unique(v8$llave))
v8 %>% filter(is.na(p04)) %>% View()

v8 %>% filter(is.na(p04)) %>% group_by(p01) %>% count()
v8 %>% filter(is.na(p04)) %>% group_by(p02) %>% count()
# asegurarse que el nombre de las variables para la unión sean los mismo y del mismo tipo
bj1<-aux1 %>% inner_join(v8) # 1:1
bj2<-aux1 %>% left_join(v8) # izquierda
bj3<-aux1 %>% right_join(v8) # derecha
bj4<-aux1 %>% full_join(v8) # todos contra todos
bj5<-aux1 %>% full_join(v8 %>% rename(id=llave)) # todos contra todos
library(labelled)
library(xtable)

bj1 %>% mutate(revoque=(p04==1)) %>% group_by(Sexo=p24) %>% summarise(Indicador=mean(revoque)*100)

bj1 %>% mutate(revoque=(p04==1)) %>% group_by(Sexo=to_factor(p24)) %>% summarise(Indicador=mean(revoque)*100)

bj1 %>% mutate(revoque=(p04==1)) %>% group_by(Sexo=to_factor(p24)) %>% summarise(Indicador=mean(revoque)*100) %>% xtable()

bj1 %>% mutate(revoque=(p04==1),aux=1) %>% group_by(Sexo=to_factor(p24)) %>% summarise(Indicador=mean(revoque)*100, Viviendas=n()) %>% mutate(Porcentaje=(Viviendas/sum(Viviendas))*100)
```
